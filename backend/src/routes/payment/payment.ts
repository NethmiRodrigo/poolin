...
export const fetchTotalSale = async (req: Request, res: Response) => {
  // const { id, verified } = req.params;
  ...
  .select('SUM(payment.totalIncome)', 'totalIncome',)
  // .where('payment.paymentDate > CURRENT_DATE - 7')
  // .groupBy('DATE_TRUNC MONTH(payment.totalIncome)')
  // .groupBy(DATE_TRUNC('month','production_timestamp)')
  .getRawMany();
  
  return res.json({ totalIncome });
};

export const getAllPayments = async (_, res: Response) => {
